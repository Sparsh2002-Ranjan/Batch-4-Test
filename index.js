...
async function fetchAndSaveData() {
  try {
    ...
    const hostler = fs.readFileSync('hostler.txt', 'utf-8').split('\n').map(line => line.trim()).filter(Boolean);
    ...
    for (let i = 0; i < rolls.length; i++) {
      ...
      const hostler =hostler[i];
      let studentData = { roll, name, url, section, hostler };

      console.log(`Processing data for roll number: ${roll}, name: ${name}, section: ${section}, hostler: ${hostler}`);
